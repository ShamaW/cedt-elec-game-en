...
bool isPlayingSurprised = false;
float surpriseAnimationStartTime = 0.0f;

// Dance animation state
bool isPlayingDance = false;
float danceAnimationStartTime = 0.0f;
...
// Array to track which items are collected
...
int main()
{
    ...
    Animation walkAnimation(FileSystem::getPath("resources/objects/remy/remy_walking.dae"), &ourModel);
    Animation idleAnimation(FileSystem::getPath("resources/objects/remy/remy_idle.dae"), &ourModel);
    Animation surprisedAnimation(FileSystem::getPath("resources/objects/remy/remy_surprised.dae"), &ourModel);
    Animation danceAnimation(FileSystem::getPath("resources/objects/remy/remy_dance.dae"), &ourModel);
    ...
    // Set the skybox shader uniform once
    skyboxShader.use();
    ...
    std::cout << "  T          - Special dancing from Remy" << std::endl;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        float rotationLerpSpeed = 6.0f * deltaTime;

        // Find the difference
        float diff = targetYaw - currentYaw;

        // Find the shortest path around the circle (to prevent spinning the long way)
        ...
        currentYaw += diff * rotationLerpSpeed;

        animator.UpdateAnimation(deltaTime);

        // NEW: Check for 'T' key press to trigger dance
        // We only trigger if no other special animation is playing
        if (glfwGetKey(window, GLFW_KEY_T) == GLFW_PRESS && !isPlayingSurprised && !isPlayingDance)
        {
			std::cout << "Remy starts dancing!" << std::endl;
            animator.PlayAnimation(&danceAnimation);
            isPlayingDance = true;
            danceAnimationStartTime = (float)glfwGetTime();
        }

        // Check if we are playing the special "surprised" animation (HIGHEST priority)
        if (isPlayingSurprised)
        {
            float animationDuration = surprisedAnimation.GetDuration();

            // Check if the animation has finished
            if ((float)glfwGetTime() - surpriseAnimationStartTime >= animationDuration)
            {
                // Animation is done, go back to normal logic
                isPlayingSurprised = false;

                // Force logic to immediately switch to correct idle/walk animation
                wasMovingLastFrame = !isMoving;
            }
        }
        // NEW: Check for "dance" animation (SECOND priority)
        else if (isPlayingDance)
        {
            float animationDuration = danceAnimation.GetDuration();

            // Check if the animation has finished
            if ((float)glfwGetTime() - danceAnimationStartTime >= animationDuration)
            {
                // Animation is done, go back to normal logic
        ...
        // If no special animation is playing, do normal idle/walk logic
        else
        {
            if (isMoving)
            {
                // If we WEREN'T moving last frame, but ARE now...
                if (!wasMovingLastFrame)
                {
                    animator.PlayAnimation(&walkAnimation); // ...play the walk animation
                    wasMovingLastFrame = true; // ...and remember we're now moving
                }
            }
            else
            {
                // If we WERE moving last frame, but AREN'T now...
                if (wasMovingLastFrame)
                {
                    animator.PlayAnimation(&idleAnimation); // ...play the idle animation
                    wasMovingLastFrame = false; // ...and remember we're now idle
                }
            }
        }
        ...
        for (int i = 0; i < NUM_ITEMS; i++)
        {
            ...
            if (!itemCollected[i] && checkCollision(playerPos, playerSize, itemPositions[i], itemSize))
            {
                ...
                {
                    std::cout << "Remy is surprised! You have collect every gear now!" << std::endl;
                    std::cout << "That's all of them! Now, find the door (it should be cyan)!" << std::endl;

                    // --- NEW: Trigger the surprised animation ---
                    animator.PlayAnimation(&surprisedAnimation);
                    isPlayingSurprised = true;
                    surpriseAnimationStartTime = (float)glfwGetTime();
                    // ---------------------------------------------
                }
            }
        }
        ...
        model = glm::rotate(model, currentYaw, glm::vec3(0.0f, 1.0f, 0.0f));
